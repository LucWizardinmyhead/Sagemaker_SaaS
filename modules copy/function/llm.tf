...
resource "aws_iam_role" "lambda_execution_role" {
  ...
  name = local.ShouldNotCreateEnvResources ? "turnkeyLambdaRole7a7e6739" : join("", ["turnkeyLambdaRole7a7e6739", "-", var.env])
}
...
resource "aws_lambda_function" "lambda_function" {
  ...
  environment {
    variables = {
      ...
      ENDPOINT_NAME_LLM = var.endpoint_name_llm
      ENDPOINT_NAME_SAGEMAKER = var.endpoint_name_sagemaker
      ENV = var.env
      INFERENCE_NAME_LLM = var.inference_name_llm
      OPENAI_API_KEY = var.openai_api_key
      REGION = data.aws_region.current.name
      REGION_NAME_LLM = var.region_name_llm
      REGION_NAME_SAGEMAKER = var.region_name_sagemaker
    }
  }
  function_name = local.ShouldNotCreateEnvResources ? "LLM" : join("", ["LLM", "-", var.env])
