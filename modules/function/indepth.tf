...
resource "aws_iam_role" "lambda_execution_role" {
  ...
  name = local.ShouldNotCreateEnvResources ? "turnkeyLambdaRole98f8a2a0" : join("", ["turnkeyLambdaRole98f8a2a0", "-", var.env])
}
...
resource "aws_lambda_function" "lambda_function" {
  ...
  function_name = local.ShouldNotCreateEnvResources ? "Indepth" : join("", ["Indepth", "-", var.env])
