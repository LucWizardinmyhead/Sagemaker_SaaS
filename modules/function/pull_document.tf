...
resource "aws_iam_role" "lambda_execution_role" {
  ...
  name = local.ShouldNotCreateEnvResources ? "turnkeyLambdaRoled1c2d178" : join("", ["turnkeyLambdaRoled1c2d178", "-", var.env])
}
...
resource "aws_lambda_function" "lambda_function" {
  ...
  function_name = local.ShouldNotCreateEnvResources ? "PullDocument" : join("", ["PullDocument", "-", var.env])
