...
resource "aws_iam_role" "lambda_execution_role" {
  ...
  name = local.ShouldNotCreateEnvResources ? "turnkeyLambdaRoleefdc10bd" : join("", ["turnkeyLambdaRoleefdc10bd", "-", var.env])
}
...
resource "aws_lambda_function" "lambda_function" {
  ...
  function_name = local.ShouldNotCreateEnvResources ? "ReturnInfo" : join("", ["ReturnInfo", "-", var.env])
