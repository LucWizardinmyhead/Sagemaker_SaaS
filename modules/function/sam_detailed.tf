...
resource "aws_iam_role" "lambda_execution_role" {
  ...
  name = local.ShouldNotCreateEnvResources ? "turnkeyLambdaRole193d4272" : join("", ["turnkeyLambdaRole193d4272", "-", var.env])
}
...
resource "aws_lambda_function" "lambda_function" {
  ...
  function_name = local.ShouldNotCreateEnvResources ? "SAMDetailed" : join("", ["SAMDetailed", "-", var.env])
